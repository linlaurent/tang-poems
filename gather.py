...
def fetch_from_gushi_ci_api() -> list:
    """从 gushi.ci API 获取唐诗"""
    try:
...
def fetch_from_github_dataset() -> list:
    """从 GitHub 中文诗歌数据集获取唐诗三百首"""
    try:
        print("正在从 GitHub 中文诗歌数据集获取唐诗...")
        tang_poems = []
        ...
        # 尝试获取多个文件以收集足够的诗歌
        for i in range(20):  # 尝试前20个文件
            try:
                url = f"{base_url}{i}.json"
                print(f"  正在获取文件 {i}...", end='\r')
        ...
        print(f"\n从 GitHub 数据集获取到 {len(unique_poems)} 首唐诗")
...
def download_tangshi():
    ...
    # 首先尝试从 gushi.ci API 获取
    poems = fetch_from_gushi_ci_api()
    
    # 如果不够，尝试从 GitHub 获取
    if len(poems) < 50:
        github_poems = fetch_from_github_dataset()
        if github_poems:
            poems = github_poems
    ...
    # 保存为 JSON 文件
    json_file = data_dir / "tang_poems.json"
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(poems, f, ensure_ascii=False, indent=2)
    
    # 保存为 TXT 文件（更易读）
    txt_file = data_dir / "tang_poems.txt"
